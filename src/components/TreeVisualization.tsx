...
export default function TreeVisualization({ step }: Props) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full bg-slate-950 rounded-xl overflow-hidden cursor-grab active:cursor-grabbing select-none"
    ...
    >
      <svg width="100%" height="100%" className="block">
        ...
        >
          {/* Suffix links — curved arcs that swing to the right to avoid tree edges */}
          {layout.suffixLinks.map((sl, idx) => {
            // Self-link (root → root): draw a small loop to the right
            if (sl.from === sl.to) {
              const loopR = 22;
              const cx = sl.fromX + NODE_RADIUS + loopR + 4;
              const cy = sl.fromY;
              // Start at right edge of node, loop out and back
            ...
            // Always swing the arc to the RIGHT of the source node
            ...
            // Start point: from source toward control point, at circle edge
            ...
            // End point: from control point into target, at circle edge
          ...
          {layout.edges.map((edge) => {
            ...
            // Place label at 70% along the edge (closer to child) so sibling
            // labels spread out horizontally instead of clustering near the parent
            const t = 0.7;
            ...
            return (
              <g key={`e-${edge.from}-${edge.to}`}>
                <line
                  ...
                  className="transition-all duration-500"
                />
                {/* Dark background behind label text */}
                <rect
